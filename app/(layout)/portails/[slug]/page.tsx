...
import SimilarPortailsSection from '@/features/portails/similarPortailsSection';
import PortailTypesSection from '@/features/portails/portailTypesSection';
import { 
  Star, 
  Shield, 
  Home, 
...
const PortailDetailPage = () => {
  const params = useParams();
  const router = useRouter();
  const { data: session } = useSession();
  const [portail, setPortail] = useState<Product | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [activeTab, setActiveTab] = useState<'description' | 'caracteristiques' | 'dimensions'>('description');

  useEffect(() => {
    const fetchPortail = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/products?type=PORTAIL');
        if (!response.ok) throw new Error('Failed to fetch products');
        
        const data = await response.json();
        
        const allowedCategories = ["PORTAIL", "PORTILLON", "CLOTURE", "GARDE_CORPS"];
        const portailProducts = data.products.filter((p: Product) => 
          allowedCategories.includes(p.category)
        );
        
        const foundPortail = portailProducts.find((p: Product) => 
  ...
  if (error || !portail) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 bg-white">
        ...
        <Button onClick={() => router.push('/portails')} variant="outline">
  ...
  return (
    <div className="min-h-screen bg-white -mt-8 lg:-mt-12">
      ...
      <div className="border-b bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <Link href="/portails" className="text-gray-600 hover:text-primary transition-colors">
              Portails & Clôtures
            </Link>
            <span className="text-gray-400">/</span>
            <span className="text-gray-900 font-medium">{portail.name}</span>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16">
          ...
          <div className="space-y-6">
            <div className="relative bg-gray-50 rounded-sm overflow-hidden border">
              ...
              {/* Badges overlay */}
              {((portail.isNew ?? false) || (portail.isPopular ?? false)) && (
            ...
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                {portail.rating && (
                  <div className="flex items-center gap-1 px-2 py-1 bg-amber-50 border border-amber-200">
                    ...
                    <span className="text-sm font-semibold text-amber-700">{portail.rating}</span>
                  </div>
                )}
              </div>
              ...
              {portail.seller && (
                <p className="text-sm text-gray-600">
                  Fabriqué par <span className="font-semibold text-primary">{portail.seller}</span>
                </p>
              )}
            </div>
              ...
              <div className="space-y-6">
                {activeTab === 'description' && (
                  <div className="prose prose-sm max-w-none">
                    <p className="text-gray-700 leading-relaxed">{portail.description}</p>
                  </div>
                )}
                      ...
                      <div className="mt-6 pt-6 border-t">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-4 bg-orange-50 border border-orange-200">
                            ...
                            <p className="text-sm font-bold text-orange-900">{portail.performance}</p>
                          </div>
                          ...
                          <div className="p-4 bg-blue-50 border border-blue-200">
                            ...
                            <p className="text-sm font-bold text-blue-900">Renforcée</p>
                          </div>
                          ...
                          <div className="p-4 bg-green-50 border border-green-200">
                            ...
                            <p className="text-sm font-bold text-green-900">Compatible</p>
                          </div>
                          ...
                          <div className="p-4 bg-purple-50 border border-purple-200">
                            ...
                            <p className="text-sm font-bold text-purple-900">10 ans</p>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {portail.dimensions && (
                        <div className="p-4 bg-gray-50 border">
                          ...
                          <p className="text-sm font-bold text-gray-900">{portail.dimensions}</p>
                        </div>
                      )}
                      ...
                      {portail.epaisseur && (
                        <div className="p-4 bg-gray-50 border">
                          ...
                          <p className="text-sm font-bold text-gray-900">{portail.epaisseur}</p>
                        </div>
                      )}
                    </div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="text-center">
              ...
              <h3 className="text-base font-bold text-gray-900 mb-2">Motorisation disponible</h3>
              <p className="text-sm text-gray-600">
      ...
      <SimilarPortailsSection currentProduct={portail} />
    </div>
  );
};

export default PortailDetailPage;