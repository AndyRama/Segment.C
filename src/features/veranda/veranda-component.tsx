...
import { X, Star, Shield, Phone, Mail, MapPin, Sun, Home, TreePine } from "lucide-react";
...
const VerandasSection = ({ className }: VerandasSectionProps) => {
	const [selectedVeranda, setSelectedVeranda] = useState<VerandaProps | null>(null);
	...
	const verandas: VerandaProps[] = [
		{
			...
			features: ["Toiture vitrée", "Isolation renforcée", "Ventilation naturelle", "Éclairage optimal"],
			description: "Véranda aluminium contemporaine avec toiture entièrement vitrée. Créez votre jardin d'hiver pour profiter de la nature toute l'année.",
		...
		},
		{
			...
			features: ["Toiture tuile", "Style authentique", "Isolation thermique", "Prix accessible"],
			description: "Véranda PVC au style traditionnel avec toiture tuile, parfaitement intégrée à l'architecture classique de votre maison.",
		...
		},
		{
			...
			features: ["Bois massif", "Toiture mixte", "Charme authentique", "Isolation naturelle"],
			description: "Véranda en bois massif au charme intemporel. Matériau noble pour une extension harmonieuse et chaleureuse.",
		...
		},
		{
			...
			features: ["Design épuré", "Toiture verre", "Grandes ouvertures", "Performance énergétique"],
			description: "Véranda aluminium au design contemporain avec un maximum de vitrage. Architecture moderne pour une luminosité exceptionnelle.",
		...
		},
		{
			...
			features: ["Lames orientables", "Gestion climatique", "Protection solaire", "Ventilation automatique"],
			description: "Véranda bioclimatique avec toiture à lames orientables. Contrôlez parfaitement température et luminosité selon les saisons.",
		...
		},
		{
			...
			features: ["Style cottage", "Toiture polycarbonate", "Isolation correcte", "Entretien minimal"],
			description: "Véranda PVC au style cottage anglais avec toiture polycarbonate. Charme britannique pour une extension cosy et lumineuse.",
		...
		},
		{
			...
			features: ["Matériaux mixtes", "Toiture vitrée", "Haute performance", "Design sophistiqué"],
			description: "Véranda mixte bois-aluminium alliant esthétique du bois et performance de l'aluminium. Excellence technique et visuelle.",
		...
		},
		{
			...
			features: ["Grande surface", "Toiture mixte", "Isolation renforcée", "Chauffage intégré"],
			description: "Grande véranda familiale pour agrandir votre espace de vie. Conception adaptée aux grandes réunions de famille.",
		...
		},
		{
			...
			features: ["Ambiance spa", "Toiture isolante", "Intimité préservée", "Équipements wellness"],
			description: "Véranda dédiée au bien-être avec toiture opaque pour créer une ambiance spa relaxante et intimiste.",
		...
		},
		{
			...
			features: ["Style atelier", "Toiture verrière", "Luminosité nordique", "Structure robuste"],
			description: "Véranda style atelier d'artiste avec structure acier et verrière. Lumière parfaite pour la création et les activités artistiques.",
		...
		},
		{
			...
			features: ["Excellent rapport qualité-prix", "Installation rapide", "Entretien facile", "Garantie étendue"],
			description: "Véranda PVC économique sans compromis sur la qualité. Solution accessible pour agrandir votre espace de vie.",
		...
		},
		{
			...
			features: ["Esprit loft", "Structure apparente", "Toiture mixte", "Design urbain"],
			description: "Véranda style loft industriel avec structure acier apparente. Extension moderne pour les architectures urbaines contemporaines.",
			priceRange: "20000€ - 35000€",
			rating: 4.6,
			surface: "20 à 40 m²"
		}
	...
	const filteredVerandas = verandas.filter(veranda => {
		return (filters.material === "all" || veranda.material === filters.material) &&
...
const VerandasHeader = () => (
	<div className="mb-12 space-y-4 text-center">
		...
		<Typography variant="large" className="mx-auto max-w-3xl text-muted-foreground">
			Agrandissez votre espace de vie avec nos vérandas sur mesure. Jardin d'hiver, extension familiale
			ou espace détente, créez votre pièce supplémentaire baignée de lumière naturelle.
		</Typography>
	</div>
);
...
const VerandasFilters = ({
	...
	<div className="mb-8 space-y-4">
		...
		<div className="flex flex-wrap justify-center gap-2">
			<span className="mr-2 self-center text-sm font-medium text-muted-foreground">Toiture:</span>
			...
			<div className="relative overflow-hidden rounded-lg bg-white shadow-md transition-all duration-300 hover:shadow-xl">
				...
				<div className="space-y-3 p-4">
					<div className="flex items-center justify-between">
						<Typography variant="large" className="font-semibold">{veranda.name}</Typography>
					...
					<div className="flex items-center justify-between pt-2">
						<Typography variant="small" className="font-semibold text-primary">{veranda.priceRange}</Typography>
...
const VerandaModal = ({
	...
	const getPerformanceIcon = (feature: string) => {
		...
		if (feature.includes('naturelle') || feature.includes('Bois') || feature.includes('Charme')) return TreePine;
						...
						{!showQuoteForm ? (
							<>
								...
								<div className="grid grid-cols-2 gap-4 rounded-lg bg-gray-50 p-4">
									<div>
										<Typography variant="small" className="text-muted-foreground">Surface</Typography>
										<Typography variant="small" className="font-semibold">{veranda.surface}</Typography>
									</div>
									<div>
										<Typography variant="small" className="text-muted-foreground">Matériau</Typography>
										<Typography variant="small" className="font-semibold capitalize">{veranda.material}</Typography>
									</div>
									<div>
										<Typography variant="small" className="text-muted-foreground">Style</Typography>
										<Typography variant="small" className="font-semibold capitalize">{veranda.style}</Typography>
									</div>
									<div>
										<Typography variant="small" className="text-muted-foreground">Toiture</Typography>
										<Typography variant="small" className="font-semibold capitalize">{veranda.toiture}</Typography>
									</div>
								</div>
								...
								<div className="mt-6 rounded-lg bg-gray-50 p-4">
									...
									<div className="mt-3 pt-3 border-t border-gray-200">
										<Typography variant="small" className="text-muted-foreground">
											🏠 Étude de faisabilité gratuite<br />
											📐 Plans et 3D personnalisés<br />
